...
{
    int left = 529, top = 45, color;
    color = getcolor();

    setcolor(LIGHTCYAN); 
    rectangle(525, 40, 633, 315);
    ...
    outtextxy(530,255, "Save");
    outtextxy(530,285, "Load");
...
char change_shape(int x, int y)
{
    if (x > 529 && x < 625)
    {
        if (y > 45 && y < 70) return 'b';
        if (y > 75 && y < 100) return 'l';
        if (y > 105 && y < 130) return 'p';
        if (y > 135 && y < 160) return 'e';
        if (y > 165 && y < 190) return 'f';
        if (y > 195 && y < 220) return 'r';
        if (y > 225 && y < 250) return 'c';
        if (y > 255 && y < 280) return 's';
        if (y > 285 && y < 310) return 'o';
    }
    return 0;
}
...
void main()
{
    ...
    outtextxy(maxx/2 - 90, maxy - 28, "Press Esc to Exit");
    ...
    while(1)
    {
        ...
        if (button == 1)
        {
            if (x > 4 && x < 635 && y > 431 && y < 457)
                change_color(x, y);
            else if (x > 529 && x < 635 && y > 40 && y < 310)
            {
                char newch = change_shape(x, y);
                if (newch == 's') { save_file(); }
                else if (newch == 'o') { load_file(); }
                else if (newch != 0) { ch = newch; }
            }
